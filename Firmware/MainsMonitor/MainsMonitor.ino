...
Adafruit_ADS1115 ads(ADC_I2C_ADDRESS);
...
void setup(void) {
	...
	// attach interrupts
	attachInterrupt(ELECTRIC_METER_PIN, ElectricMeter_Triggered, RISING);
	attachInterrupt(GAS_METER_PIN, GasMeter_Triggered, RISING);
	...
	// start timer
	SampleTicker.attach(1.0, GetADCData);
...
void loop(void) {
	...
	if (doADCRead == 1) {
		...
		ADCVoltage = (ADCraw * 0.26315) / 1000; // get adc voltage
		ADCVoltage = ADCVoltage * 0.95; // error correction for voltage divider
		MainsCurrent = ADCVoltage / 0.05; // calculate current from sensor, 100A = 5V
		//MainsCurrent = adc0;
...
void BatteryMonitor_Triggered(void){

}

void ElectricMeter_Triggered(void){
...
void GetADCData(void){
	...
	// toggle led to show the adc ticker is running
	digitalWrite(LED_PIN, !digitalRead(LED_PIN));
	
	doADCRead = 1; // set flag so ADC will be read on next program loop
}
